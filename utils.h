...
#define X99(sub, obj)   ( ((!IS_AFFECTED((obj),AFF_INVISIBLE) || \
 ...
 ((sub)->equipment[WEAR_FACE] && \
 ((sub)->equipment[WEAR_FACE]->obj_flags.value[2]==2))))
...
#define GET_SPELL_LEV_BONUS(ch)   ((ch)->specials.spell_lev_bonus)
#define GET_SPELL_LEVEL(ch)   (GET_LEVEL(ch) + GET_SPELL_LEV_BONUS(ch))
...
#define GET_META(ch)     ((ch)->points.metapts)
#define GET_META_BONUS(ch)  ((ch)->specials.metabonus)
#define GET_HEIGHT(ch)  ((ch)->player.height)
#define GET_WEIGHT(ch)  ((ch)->player.weight)
#define GET_SEX(ch)     ((ch)->player.sex)
#define GET_GUT(ch)     ((ch)->specials.gut)
#define GET_HITROLL(ch) ((ch)->points.hitroll)
#define GET_DAMROLL(ch) ((ch)->points.damroll)
#define AWAKE(ch) (GET_POS(ch) > POSITION_SLEEPING)

#define WAIT_STATE(X,Y) (IS_AFFECTED(X,AFF_HASTE) ? W_S(X,(Y)>>1) : W_S(X,Y))
#define W_S(ch,cycle)  ((((ch)->desc) ? (ch)->desc->wait = (cycle) : 0))
...
#define CAN_CARRY_W(ch) (GET_STR(ch) * 12)
...
#define GET_SAVING_THROW(lev) (IMO-lev)

#define XP4LEV(X) ((((X)*(X)*(X)) < 0) ? 0x7fff0000 : ((X)*(X)*(X)))

#define CANFARSEE(X) ((GET_LEVEL(X) >= 60)|| IS_AFFECTED(X,AFF_FARSEE) || \
 ((X->equipment[WEAR_FACE])&&(X->equipment[WEAR_FACE]->obj_flags.value[2] & 1)))

#define CANINFRA(X) (IS_AFFECTED(X,AFF_INFRAVISION) || \
 ((X->equipment[WEAR_FACE])&&(X->equipment[WEAR_FACE]->obj_flags.value[2] & 2)))

#define CANSEEINV(X) ((X->equipment[WEAR_FACE]) && \
 (X->equipment[WEAR_FACE]->obj_flags.value[2] & 4))

#define CANSEEHID(X) (IS_AFFECTED(X,AFF_SENSE_LIFE) || \
 ((X->equipment[WEAR_FACE])&&(X->equipment[WEAR_FACE]->obj_flags.value[2] & 8)))
