...
*/
{
  ...
  rm=ch->in_room;
  if(rm >= 0)
    need_movement=(movement_loss[world[rm].sector_type]+
      movement_loss[world[world[rm].dir_option[cmd]->to_room].sector_type])>>1;
  else {
    log("Moving from negative room number.");
  }
  if((world[rm].sector_type==SECT_WATER_NOSWIM)||
     (world[world[rm].dir_option[cmd]->to_room].sector_type==
       SECT_WATER_NOSWIM)){
    has_boat = IS_SET(ch->specials.act, ACT_SWIMMER) ||
               ((GET_LEVEL(ch) >= IMO) && !IS_NPC(ch));
...
void do_move(struct char_data *ch, char *argument, int cmd)
{
  char tmp[80];
  int was_in, n;
  struct char_data *tch1, *mnt;
  struct follow_type *k, *next_dude;

  --cmd;
  if((cmd < 4) && IS_SET(world[ch->in_room].room_flags,CONFUSING) &&
     (!IS_AFFECTED(ch,AFF_CLARITY))){
    cmd=number(0,4);
    if(!CAN_GO(ch,cmd)) cmd=3-cmd;
    if(!CAN_GO(ch,cmd)){
      send_to_char("Uh, you can't go that way?\n\r",ch);
      return;
    }
  } else if(((n=GET_COND(ch,DRUNK)) > 1) && number(0,n)){
    cmd=number(0,4);
    if(!CAN_GO(ch,cmd)){
      act("$n staggers and almosts falls down.", TRUE, ch, 0,0, TO_ROOM);  
      send_to_char("You bump into something, you drunk!\n\r",ch);
      ch->specials.bonks++;	/* SLUG_CHANGE 11-17-96 */
      return;
    }
  }
  if (!world[ch->in_room].dir_option[cmd]) {
    ...
    if (IS_SET(EXIT(ch, cmd)->exit_info, EX_CLOSED)) {
      ...
      if (IS_AFFECTED(ch, AFF_CHARM) && (ch->master) && 
        ...
        send_to_char("You cannot bear to leave your master.\n\r", ch);
  ...
  if (!*type)
    ...
    if (obj->obj_flags.type_flag != ITEM_CONTAINER)
      ...
      send_to_char("Oho! This thing is NOT locked!\n\r", ch);
            ...
            IS_SET(obj->obj_flags.value[1], CONT_PICKPROOF))
